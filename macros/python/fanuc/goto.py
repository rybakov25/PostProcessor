# -*- coding: ascii -*-
# ============================================================================
# FANUC GOTO MACRO - Linear Motion for Fanuc controllers
# ============================================================================
...
def execute(context, command):
    ...
    if is_rapid:
        # Rapid move G00 (Fanuc uses G00)
        line = "G00 X" + format_num(x)
        ...
        # Linear move G01 (Fanuc uses G01)
        line = "G01 X" + format_num(x)
        if len(command.numeric) > 1:
            line += " Y" + format_num(y)
        if len(command.numeric) > 2:
            line += " Z" + format_num(z)
...
def format_num(value):
    """Format number with 3 decimal places for Fanuc"""
    rounded = round(value, 3)
    # Fanuc typically uses decimal point
    return str(rounded)
